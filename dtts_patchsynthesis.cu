...
void terrain_synthesis(char* exemplar, char* sketchf, char* output, char ridges, int bsize)
{
  ...
  /*  Display *_dsp;
   ...
   Tree features;
    ...
    //features.renderFeatures(250.).savePGM("/tmp/ridges.pgm");
    //return;

    features.runPPA(terr,true,7);
    //features.saveFile(name.c_str());
    terr.savePGM("/tmp/ridges.pgm");
    features.renderFeatures(1.).savePGM("/tmp/ridges_ppa.pgm",features.msource.getMax());
    return;*/
    ...
    res.savePGM("/tmp/res.pgm");
    cin.get();*/

    res.maxval=terrain.maxval;
    res.minval=terrain.minval;

    terrain.savePGM("/tmp/dem.pgm",terrain.maxval);
    sketch.savePGM("/tmp/user.pgm",terrain.maxval);

    terrain.saveTerragen("/tmp/dem.ter");
    sketch.saveTerragen("/tmp/user.ter");
    ...
    switch(ridges){
        ...
        case 'R':
        {
            ...
            if (usr_features.getNodes().size()>0){
                ...
                usr_features.renderFeatures(1.).savePGM("/tmp/user_ridges_gpu.pgm");
                ...
                if (fopen(name.c_str(),"r")){
                    ...
                    //dem_features.renderFeatures(1.).savePGM("/tmp/dem_ridges_gpu_loaded.pgm",terrain.maxval);
                }
                else
                {
                    dem_features.runPPA(terrain,ridges=='r',dp);
                    dem_features.saveFile(name.c_str());
                    dem_features.renderFeatures(1.).savePGM("/tmp/dem_ridges_gpu.pgm",terrain.maxval);
                }
                ...
                if (dem_features.getNodes().size()>0){
                    ...
                    res.savePGM("/tmp/res_inter_gpu.pgm",terrain.maxval);
                 }
            }
        ...
        case 'V':
        {
            ...
            if (usr_features.getNodes().size()>0){
                ...
                if (usr_features.root.x==0 && usr_features.root.y==0)   break;
                usr_features.getRoots(usr_roots);
                usr_features.renderFeatures(250.).savePGM("/tmp/user_valleys_gpu.pgm");
                ...
                if (fopen(name.c_str(),"r")){
                    dem_features.loadFile(terrain,name.c_str(),ridges=='r');
                    //dem_features.renderFeatures(250.).savePGM("/tmp/dem_valleys_loaded.pgm",terrain.maxval);
                }
                else
                {
                    dem_features.runPPA(terrain,ridges=='r',dp);
                    dem_features.saveFile(name.c_str());
                    dem_features.renderFeatures(250.).savePGM("/tmp/dem_valleys_gpu.pgm",terrain.maxval);
                }
    ...
    res.saveTerragen("/tmp/res_inter_gpu.ter");

    //return;

    //bigtex.savePGM("/tmp/big1.pgm",res.maxval);
    Image tmp = res;
    ...
    tmp.savePGM("/tmp/res_inter2_gpu.pgm");
    ...
    res.savePGM("/tmp/res_tmp_gpu.pgm");
    res.saveTerragen("/tmp/res_tmp_gpu.ter");

    //bigtex.savePGM("/tmp/big2.pgm",res.maxval);
