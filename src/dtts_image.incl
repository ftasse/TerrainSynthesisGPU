...
Dtts::Terrain::Terrain(int pwidth, int pheight)
{
    // Allocate memory for this image
...
bool Dtts::Terrain::loadTerragen(const char* fname)
{
    ...
    for(int j = getHeight()-1; j >0; j--)
        ...
        {
            //printf("elevation: %d\n",c);
            float alt = mbaseheight+((* ((short *) &inbuffer[pos]))*mheightscale/65536.0);
            //cout<<alt<<" ";
            if (alt>maxval){
                 maxval = alt;
                 //cout<<maxval<<" ";
...
bool Dtts::Terrain::saveTerragen(const char* fname)
{
    ...
    if(outfile.is_open())
    {
        ...
        val = (short) (mheightscale-1);
        outfile.write((char *) &val, 2); // terrain height
        val = (short) mbaseheight;
